...
# Consensus grid
# ---------------------------------------------------------------------------
# Every score that can influence a persisted result is snapped onto a coarse
# grid *before* validators compare answers, and the equivalence principle then
# demands byte-identical values on that grid. Two consequences:
#   * validators can never persist different numbers for the same evaluation;
#   * every downstream threshold (delta classification, decay, staleness,
#     challenge outcome) is aligned to the grid, so no admissible disagreement
#     inside a bucket can flip a delta, a timeline entry, a decay step or a
#     challenge verdict. Legal variation is variation with zero consequence.
SCORE_BUCKET = 10

# Grid-aligned thresholds: differences are always multiples of SCORE_BUCKET,
# so no pair of accepted answers can land on opposite sides of one.
STRENGTHEN_DELTA = 10
WEAKEN_DELTA = -20
WEAK_EVIDENCE_FLOOR = 40  # below this, VERIFIED/REJECTED is downgraded
DISPUTE_TRUTH_FLOOR = 70
UNCERTAIN_TRUTH_CAP = 60
STALE_TRUTH_FLOOR = 50

# The fields that actually reach storage and drive every later computation.
# Validators must agree on these exactly; everything else is prose.
CONSENSUS_FIELDS = (
    "status",
    "truth_confidence",
    "evidence_confidence",
...
EQ_PRINCIPLE = (
    "Each answer is one JSON object that already carries a `consensus` block and a "
    "`consensus_digest` string. The two answers agree if and only if their "
    "`consensus_digest` values are byte-identical, which is equivalent to every field of "
    "the `consensus` block being exactly equal: same `status`, the same integer "
    "`truth_confidence`, `evidence_confidence`, `source_quality`, `freshness`, "
    "`independence` and `independent_sources` (all reported on a fixed "
    f"{SCORE_BUCKET}-point grid, so a genuine agreement produces identical numbers, not "
    "merely close ones), the same `reality_changed` boolean, the same `change_cause`, and "
    "the same `transition_datable` boolean. Any difference in these values, however small, "
    "is a disagreement: they are the values written to the ledger and they decide the "
    "recorded change delta, the timeline entry, confidence decay and challenge outcomes. "
    "Everything outside the `consensus` block — wording of the explanation, ordering or "
    "phrasing of evidence, narrative, dates quoted in prose — is irrelevant and must not "
    "cause a rejection."
)
...
def quantize_score(value: typing.Any) -> int:
    """Snap a 0..100 score onto the consensus grid (deterministic, idempotent).

    Rounding is half-up on integers only, so every validator that reads the same
    raw score produces the same bucket, and any two accepted answers differ by a
    whole number of buckets — never by an amount that could straddle a threshold.
    """
    n = clamp_score(value)
    bucket = (n + SCORE_BUCKET // 2) // SCORE_BUCKET
    snapped = bucket * SCORE_BUCKET
    if snapped > 100:
        return 100
    if snapped < 0:
        return 0
...
def sanitize_evaluation(raw_json: str) -> dict:
    ...
    # Grid-snapped immediately: every score that can reach storage or feed a
    # threshold exists only as a bucket value from here on.
    truth = quantize_score(data.get("truth_confidence"))
    evidence = quantize_score(data.get("evidence_confidence"))
    source_quality = quantize_score(data.get("source_quality"))
    freshness = quantize_score(data.get("freshness"))
    independence = quantize_score(data.get("independence"))
    ...
    # All bounds sit on the grid, so a guard fires identically for every
    # validator whose answer was accepted.
    if evidence < WEAK_EVIDENCE_FLOOR and status in ("VERIFIED", "REJECTED"):
        status = "UNCERTAIN"
    if contradicting and supporting and status == "VERIFIED" and truth < DISPUTE_TRUTH_FLOOR:
        status = "DISPUTED"
    if status == "UNCERTAIN":
        truth = min(truth, UNCERTAIN_TRUTH_CAP)
    ...
    # A change cause is part of the consequential state, so it may not float
    # free of the judgment it explains: reality only "changed" if the answer
    # says so, and a non-reality answer may not smuggle REALITY_CHANGED in.
    if result["change_cause"] == "REALITY_CHANGED" and not result["reality_changed"]:
        result["reality_changed"] = True
    if result["reality_changed"] and result["change_cause"] == "NONE":
        result["change_cause"] = "REALITY_CHANGED"
    result["consensus"] = consensus_payload(result)
    result["consensus_digest"] = consensus_digest(result)
    return result


def consensus_payload(judgment: dict) -> dict:
    """The subset of an evaluation that validators must agree on exactly."""
    return {field: judgment.get(field) for field in CONSENSUS_FIELDS}


def consensus_digest(judgment: dict) -> str:
    """Stable fingerprint of the consequential values (comparison shortcut)."""
    canonical = json.dumps(consensus_payload(judgment), sort_keys=True, separators=(",", ":"))
    return digest_hex(canonical)[:32]


def canonical_answer_json(judgment: dict) -> str:
    """Byte-stable rendering of a sanitized judgment, for validator comparison."""
    payload = dict(judgment)
    payload["consensus"] = consensus_payload(judgment)
    payload["consensus_digest"] = consensus_digest(judgment)
...
def classify_delta(previous: dict, current: dict) -> str:
    ...
    # Both sides are grid values, so the comparison below is bucket arithmetic:
    # any two answers a validator set could have accepted give the same delta.
    prev_truth = quantize_score(previous.get("truth_confidence", 0))
    new_truth = quantize_score(current.get("truth_confidence", 0))
    ...
    if prev_status in ("DISPUTED", "UNCERTAIN", "STALE") and new_status in ("VERIFIED", "REJECTED"):
        return "RESOLVED"
    if delta_truth >= STRENGTHEN_DELTA and new_status == prev_status:
        return "STRENGTHENED"
    if delta_truth <= WEAKEN_DELTA:
        return "WEAKENED"
...
def decayed_confidence(truth: int, age: int, recheck: int, evidence_conf: int) -> int:
    ...
    Inputs and output stay on the consensus grid: decay reads only agreed
    bucket values and writes back a bucket value, so it can never turn an
    admissible validator difference into a different stored confidence.
    """
    base = quantize_score(truth)
    if recheck <= 0 or age <= recheck:
        return base
    overdue_periods = (age - recheck) / float(recheck)
    weakness = 1.0 + max(0.0, (60 - quantize_score(evidence_conf)) / 60.0)
    penalty = min(45.0, overdue_periods * 12.0 * weakness)
    return quantize_score(max(0, int(round(base - penalty))))
...
SCORING GRID: report every score in whole steps of {SCORE_BUCKET} (0, {SCORE_BUCKET}, {2 * SCORE_BUCKET}, ... 100).
Independent validators must produce the *same* number, not a nearby one; picking the bucket
your judgment falls in is what makes agreement possible. Do not report finer precision.

Reply with ONE JSON object and nothing else:
{{"status": "VERIFIED|CHANGED|UNCERTAIN|DISPUTED|REJECTED|STALE",
 "truth_confidence": 0-100 in steps of {SCORE_BUCKET},
 "evidence_confidence": 0-100 in steps of {SCORE_BUCKET},
 "source_quality": 0-100 in steps of {SCORE_BUCKET},
 "freshness": 0-100 in steps of {SCORE_BUCKET},
 "independence": 0-100 in steps of {SCORE_BUCKET},
...
class FactEvolution(Contract):
    ...
    def _gather_and_judge(self, claim: dict, previous: dict, evidence: list, mode: str) -> dict:
        ...
        def leaf() -> str:
            ...
            # Sanitize *inside* the leaf: each validator compares the exact,
            # grid-snapped values it would persist, not free-form model prose.
            return canonical_answer_json(sanitize_evaluation(extract_json_object(answer)))
        ...
        # Re-sanitizing is idempotent on an already-canonical answer, so the
        # values persisted below are byte-for-byte the ones validators agreed on.
    ...
    def _record_evaluation(self, claim_id: str, mode: str, trigger: str) -> dict:
        ...
        self._append_event(
            claim_id,
            {
                ...
                "consensus_digest": judgment.get("consensus_digest", ""),
    ...
    def apply_freshness_policy(self, claim_id: str) -> str:
        ...
        updated["status"] = "STALE" if adjusted < STALE_TRUTH_FLOOR else prev_status
        ...
        # Decay is deterministic, but it rewrites consequential values, so the
        # stored consensus block and digest are recomputed to match them.
        updated["consensus"] = consensus_payload(updated)
        updated["consensus_digest"] = consensus_digest(updated)
