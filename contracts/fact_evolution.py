...
# Every field the contract persists AND feeds back into a later evaluation
# prompt (see `build_prompt`'s PREVIOUS_CONTRACT_BELIEF block) must be bound
# here. An unbound field lets two validators accept mutually inconsistent
# narratives, and whichever one happens to be stored then steers every future
# re-evaluation of that claim.
EQ_PRINCIPLE = (
    ...
    "(5) neither claims certainty that the other calls unresolved, "
    "(6) their `temporal_scope` values describe the same period and the same kind "
    "of assertion (present-tense, a specific past window, or future): a scope that "
    "covers a different period, or that treats the claim as being about now when the "
    "other treats it as being about a past window, is a disagreement, "
    "and (7) their `explanation` fields assert the same substance — the same verdict, "
    "the same reason for it, the same dates and transitions, and the same account of "
    "what (if anything) changed; one explanation must not assert a fact, date or cause "
    "that the other denies or contradicts. "
    "Differences in wording, length, ordering of evidence, or style of the "
    "explanation and the phrasing of the temporal scope are irrelevant; only "
    "differences in asserted content count."
)
