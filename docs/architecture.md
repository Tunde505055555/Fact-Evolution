...
| agreement between validators | `gl.eq_principle_prompt_comparative` with an explicit principle | every consequential value must match exactly on the consensus grid |
...
The equivalence principle (see `EQ_PRINCIPLE` in the contract) binds the values that are
actually persisted. Each validator sanitizes its own answer inside the leaf, so every score
is snapped onto a fixed `SCORE_BUCKET` (10-point) grid, and the answers carry a `consensus`
block plus a `consensus_digest`. Two answers agree only when that digest is byte-identical —
same status, same grid-value confidences, same `reality_changed`, `change_cause`,
`transition_datable` and independent-source count. Wording, ordering of evidence and
narrative phrasing remain irrelevant.

Because thresholds are grid-aligned too (`STRENGTHEN_DELTA` +10, `WEAKEN_DELTA` -20,
`WEAK_EVIDENCE_FLOOR` 40, `DISPUTE_TRUTH_FLOOR` 70, `UNCERTAIN_TRUTH_CAP` 60,
`STALE_TRUTH_FLOOR` 50), no admissible difference between validators can move a delta,
a timeline entry, a decay step or a challenge outcome: legal variation is variation with
zero consequence. If validators cannot converge, the contract records `UNCERTAIN` with an
explanation — it never manufactures certainty, and appeals happen through GenLayer's own
consensus machinery.
